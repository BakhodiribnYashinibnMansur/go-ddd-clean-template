...
func (c *Client) SendMessage(msgType MessageType, text string) error {
	...
	if err := c.telegramCB.ExecuteWithFallback(
		func() error {
...
func (c *Client) doSend(msgType MessageType, text string) error {
	...
	// SendMessage has no ctx parameter (legacy signature, called from async
	// task handlers and fallback paths), so we create a fresh root context.
	resp, _, err := c.http.PostJSON(context.Background(), url, "SendMessage", body)
	if err != nil {
		return fmt.Errorf("telegram.doSend.PostJSON: %w", err)
	}
...
func (c *Client) bufferToPending(msgType MessageType, text string) error {
	...
	if err != nil {
		return fmt.Errorf("telegram.bufferToPending.marshal: %w", err)
	}
	// bufferToPending is a circuit-breaker fallback; its caller (SendMessage)
	// has no ctx parameter, so Background is used to guarantee the fallback
	// buffer write is not cancelled by an upstream request.
