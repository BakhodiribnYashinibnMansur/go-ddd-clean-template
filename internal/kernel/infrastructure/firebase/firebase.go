...
func NewFirebase(ctx context.Context, logger logger.Log, cfg config.Firebase, rdb redis.Cmdable) (*Firebase, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("firebase.NewFirebase.mobile.marshal: %w", err)
	}
	mobileOpt := option.WithCredentialsJSON(mobileCredentials)
	mobileApp, err := firebase.NewApp(ctx, nil, mobileOpt)
	if err != nil {
		return nil, fmt.Errorf("firebase.NewFirebase.mobile.NewApp: %w", err)
	}

	// Initialize Web Firebase app
	webCredentials, err := json.Marshal(cfg.Web)
	if err != nil {
		return nil, fmt.Errorf("firebase.NewFirebase.web.marshal: %w", err)
	}
	webOpt := option.WithCredentialsJSON(webCredentials)
	webApp, err := firebase.NewApp(ctx, nil, webOpt)
	if err != nil {
		return nil, fmt.Errorf("firebase.NewFirebase.web.NewApp: %w", err)
	}

	// Get FCM clients
	mobileFcmClient, err := mobileApp.Messaging(ctx)
	if err != nil {
		return nil, fmt.Errorf("firebase.NewFirebase.mobile.Messaging: %w", err)
	}

	webFcmClient, err := webApp.Messaging(ctx)
	if err != nil {
		return nil, fmt.Errorf("firebase.NewFirebase.web.Messaging: %w", err)
	}
