...
import (
	...
	userdomain "gct/internal/user/domain"
...
type mockUserReadRepository struct {
	findUserForAuthFn func(ctx context.Context, userID uuid.UUID) (*shared.AuthUser, error)
}

func (m *mockUserReadRepository) FindByID(_ context.Context, _ uuid.UUID) (*userdomain.UserView, error) {
	return nil, nil
}
func (m *mockUserReadRepository) List(_ context.Context, _ userdomain.UsersFilter) ([]*userdomain.UserView, int64, error) {
	return nil, 0, nil
}
func (m *mockUserReadRepository) FindSessionByID(_ context.Context, _ uuid.UUID) (*shared.AuthSession, error) {
	return nil, nil
}
func (m *mockUserReadRepository) FindUserForAuth(ctx context.Context, userID uuid.UUID) (*shared.AuthUser, error) {
	if m.findUserForAuthFn != nil {
		return m.findUserForAuthFn(ctx, userID)
	}
