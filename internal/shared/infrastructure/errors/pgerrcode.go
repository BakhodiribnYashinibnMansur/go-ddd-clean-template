// Package errors contains PostgreSQL error codes constants.
// Source: https://www.postgresql.org/docs/14/errcodes-appendix.html
// Based on jackc/pgerrcode
package errors
...
// Helper functions to check error code classes

// IsIntegrityConstraintViolation checks if error code is integrity constraint violation (Class 23)
func IsIntegrityConstraintViolation(code string) bool {
	return len(code) >= 2 && code[:2] == "23"
}

// IsConnectionException checks if error code is connection exception (Class 08)
func IsConnectionException(code string) bool {
	return len(code) >= 2 && code[:2] == "08"
}

// IsDataException checks if error code is data exception (Class 22)
func IsDataException(code string) bool {
	return len(code) >= 2 && code[:2] == "22"
}

// IsInvalidTransactionState checks if error code is invalid transaction state (Class 25)
func IsInvalidTransactionState(code string) bool {
	return len(code) >= 2 && code[:2] == "25"
}

// IsTransactionRollback checks if error code is transaction rollback (Class 40)
func IsTransactionRollback(code string) bool {
	return len(code) >= 2 && code[:2] == "40"
}

// IsSyntaxErrorOrAccessRuleViolation checks if error code is syntax error or access rule violation (Class 42)
func IsSyntaxErrorOrAccessRuleViolation(code string) bool {
	return len(code) >= 2 && code[:2] == "42"
}

// IsInsufficientResources checks if error code is insufficient resources (Class 53)
func IsInsufficientResources(code string) bool {
	return len(code) >= 2 && code[:2] == "53"
}

// IsProgramLimitExceeded checks if error code is program limit exceeded (Class 54)
func IsProgramLimitExceeded(code string) bool {
	return len(code) >= 2 && code[:2] == "54"
}

// IsObjectNotInPrerequisiteState checks if error code is object not in prerequisite state (Class 55)
func IsObjectNotInPrerequisiteState(code string) bool {
	return len(code) >= 2 && code[:2] == "55"
}

// IsOperatorIntervention checks if error code is operator intervention (Class 57)
func IsOperatorIntervention(code string) bool {
	return len(code) >= 2 && code[:2] == "57"
}

// IsSystemError checks if error code is system error (Class 58)
func IsSystemError(code string) bool {
	return len(code) >= 2 && code[:2] == "58"
}

// IsForeignDataWrapperError checks if error code is foreign data wrapper error (Class HV)
func IsForeignDataWrapperError(code string) bool {
	return len(code) >= 2 && code[:2] == "HV"
}

// IsPLpgSQLError checks if error code is PL/pgSQL error (Class P0)
func IsPLpgSQLError(code string) bool {
	return len(code) >= 2 && code[:2] == "P0"
}

// IsInternalError checks if error code is internal error (Class XX)
func IsInternalError(code string) bool {
	return len(code) >= 2 && code[:2] == "XX"
}
