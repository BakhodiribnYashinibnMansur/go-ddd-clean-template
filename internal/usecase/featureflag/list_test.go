	...
	tests := []struct {
	...
	}{
		{
			...
			mockSetup: func(m *MockRepo) {
				m.On("List", mock.Anything, domain.FeatureFlagFilter{
					Limit:    10,
					Offset:   0,
					IsActive: &active,
		...
		},
		{
			...
			mockSetup: func(m *MockRepo) {
				m.On("List", mock.Anything, domain.FeatureFlagFilter{Limit: 10}).
					Return([]domain.FeatureFlag{}, int64(0), nil)
			},
			wantCount: 0,
			wantLen:   0,
			wantErr:   false,
		},
		{
			...
			mockSetup: func(m *MockRepo) {
				m.On("List", mock.Anything, domain.FeatureFlagFilter{Limit: 10}).
					Return([]domain.FeatureFlag{}, int64(0), errors.New("database error"))
			},
			wantErr: true,
		},
	}
