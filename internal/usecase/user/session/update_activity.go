...
func (uc *UseCase) UpdateActivity(ctx context.Context, in *domain.SessionFilter) error {
	uc.logger.Infow("session update activity started", "input", in)

	repo := uc.repo.Postgres.User.SessionRepo
	s, err := repo.Get(ctx, in)
	if err != nil {
		uc.logger.Errorw("session update activity failed: get", "error", err)
	...
	if s.IsExpired() || s.Revoked {
		...
		uc.logger.Errorw("session update activity failed: invalid", "error", err)
		return err
	}
	...
	if err != nil {
		uc.logger.Errorw("session update activity failed: update", "error", err)
		return apperrors.MapRepoToServiceError(ctx, err).WithInput(in)
	}

	uc.logger.Infow("session update activity success", "session_id", s.ID)
	return nil
}
