...
func (r *Repo) List(ctx context.Context, filter domain.FeatureFlagFilter) ([]domain.FeatureFlag, int64, error) {
	q := r.builder.
		Select("id", "key", "name", "type", "value", "description", "is_active", "created_at", "updated_at").
		From(table).
	...
	if filter.Search != "" {
		q = q.Where(squirrel.Or{
			...
			squirrel.ILike{"key": "%" + filter.Search + "%"},
		})
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&f.ID, &f.Key, &f.Name, &f.Type, &f.Value, &f.Description, &f.IsActive, &f.CreatedAt, &f.UpdatedAt); err != nil {
			return nil, 0, apperrors.HandlePgError(err, table, nil)
		}
