...
func (c *Controller) CsrfToken(ctx *gin.Context) {
	...
	sessionID := httpx.GetDeviceID(ctx)
	if sessionID == "" {
		sessionID = httpx.GenerateToken()
	}
	...
	if err != nil {
		httpx.LogError(c.l, err, "http - v1 - auth - csrf - generate")
	...
	if c.csrfStore != nil {
		if err := c.csrfStore.Set(ctx.Request.Context(), sessionID, token.Hash, csrf.DefaultExpiration); err != nil {
			httpx.LogError(c.l, err, "http - v1 - auth - csrf - store")
