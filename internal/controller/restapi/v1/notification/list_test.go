...
func TestList(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "success",
			query: "?limit=10&offset=0&search=system&type=info",
			mockSetup: func(m *MockUseCase) {
				m.On("List", mock.Anything, mock.AnythingOfType("domain.NotificationFilter")).
					Return([]domain.Notification{{Title: "System Update"}}, int64(1), nil)
			},
		...
		},
		{
			...
			mockSetup: func(m *MockUseCase) {
				m.On("List", mock.Anything, mock.AnythingOfType("domain.NotificationFilter")).
					Return([]domain.Notification{}, int64(0), nil)
			},
			wantStatus: http.StatusOK,
		},
		{
