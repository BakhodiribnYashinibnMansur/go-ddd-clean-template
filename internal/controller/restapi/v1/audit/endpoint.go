...
func AuditRoute(api *gin.RouterGroup, controller *Controller, authMiddleware gin.HandlerFunc) {
	log.Route(api, controller.Log, authMiddleware)
	history.Route(api, controller.History, authMiddleware)
	metric.Route(api, controller.Metric, authMiddleware)
}
