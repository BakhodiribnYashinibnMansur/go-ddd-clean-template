	...
	tests := []struct {
	...
	}{
		{
			...
			mockSetup: func(m *MockUseCase) {
				m.On("Update", mock.Anything, validID, mock.AnythingOfType("domain.UpdateRateLimitRequest")).
					Return(&domain.RateLimit{ID: validID, Name: "updated-rule"}, nil)
			},
		...
		},
		{
			...
			wantStatus: http.StatusOK, // BindJSON returns false without setting status
		},
		{
			name:    "internal_error",
			paramID: validID.String(),
