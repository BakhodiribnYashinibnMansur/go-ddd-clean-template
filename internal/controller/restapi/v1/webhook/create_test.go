...
func TestCreate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "success",
			body: `{"name":"My Webhook","url":"https://example.com/hook","secret":"s3cret","events":["user.created"],"is_active":true}`,
			mockSetup: func(m *MockUseCase) {
				m.On("Create", mock.Anything, mock.AnythingOfType("domain.CreateWebhookRequest")).
					Return(&domain.Webhook{Name: "My Webhook"}, nil)
			},
