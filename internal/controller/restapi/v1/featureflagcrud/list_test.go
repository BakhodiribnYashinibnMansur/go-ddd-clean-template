...
func TestList(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "success",
			query: "?limit=10&offset=0&search=dark",
			mockSetup: func(m *MockUseCase) {
				m.On("List", mock.Anything, mock.AnythingOfType("domain.FeatureFlagFilter")).
					Return([]domain.FeatureFlag{{Name: "Dark Mode"}}, int64(1), nil)
			},
