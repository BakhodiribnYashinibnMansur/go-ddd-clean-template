...
func TestList(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "success",
			query: "?limit=10&offset=0&search=test&status=idle",
			mockSetup: func(m *MockUseCase) {
				m.On("List", mock.Anything, mock.AnythingOfType("domain.JobFilter")).
					Return([]domain.Job{{Name: "test-job"}}, int64(1), nil)
			},
