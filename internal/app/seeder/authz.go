...
func (s *Seeder) seedRolePermissions(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("seeder.authz.query_roles: %w", err)
	}
	...
	for roleRows.Next() {
		...
		if err := roleRows.Scan(&id); err != nil {
			return fmt.Errorf("seeder.authz.scan_role: %w", err)
		}
	...
	if err != nil {
		return fmt.Errorf("seeder.authz.query_permissions: %w", err)
	}
	...
	for permRows.Next() {
		...
		if err := permRows.Scan(&id); err != nil {
			return fmt.Errorf("seeder.authz.scan_permission: %w", err)
		}
...
func (s *Seeder) seedPolicies(ctx context.Context, count int) error {
	...
	if err != nil {
		return fmt.Errorf("seeder.authz.policies.query: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id); err != nil {
			return fmt.Errorf("seeder.authz.policies.scan: %w", err)
		}
		permIDs = append(permIDs, id)
	}
