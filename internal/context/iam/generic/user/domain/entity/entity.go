...
func NewUser(phone Phone, password Password, opts ...UserOption) (*User, error) {
	...
	u.AddEvent(userevent.NewUserCreated(u.ID(), phone.Value()))
	return u, nil
}
...
func (u *User) AddSession(deviceType SessionDeviceType, ip, userAgent, integrationName string, deviceFingerprint ...string) (*Session, error) {
	...
	u.Touch()
	u.AddEvent(userevent.NewUserSignedIn(u.ID(), s.ID(), ip))
	return s, nil
}
...
func (u *User) ChangePassword(oldRaw, newRaw string) error {
	...
	u.password = pw
	u.Touch()
	u.AddEvent(userevent.NewPasswordChanged(u.ID()))
	return nil
}
...
func (u *User) Deactivate() {
	u.active = false
	u.Touch()
	u.AddEvent(userevent.NewUserDeactivated(u.ID()))
}
...
func (u *User) Approve() {
	u.isApproved = true
	u.Touch()
	u.AddEvent(userevent.NewUserApproved(u.ID()))
}
...
func (u *User) ChangeRole(roleID uuid.UUID) {
	...
	u.roleID = &roleID
	u.Touch()
	u.AddEvent(userevent.NewRoleChanged(u.ID(), old, roleID))
}
