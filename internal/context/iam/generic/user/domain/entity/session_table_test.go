...
func TestNewSession_TableDriven(t *testing.T) {
	...
	tests := []struct {
		name       string
		deviceType entity.SessionDeviceType
		ip         string
		userAgent  string
	}{
		{"desktop", entity.DeviceDesktop, "192.168.1.1", "Chrome/120"},
		{"mobile", entity.DeviceMobile, "10.0.0.1", "Safari/17"},
		{"tablet", entity.DeviceTablet, "172.16.0.1", "Firefox/119"},
		{"bot", entity.DeviceBot, "8.8.8.8", "Googlebot/2.1"},
		{"tv", entity.DeviceTV, "192.168.0.100", "SmartTV/1.0"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			s, _ := entity.NewSession(userID.UUID(), tt.deviceType, tt.ip, tt.userAgent, "gct-client")
...
func TestSession_Lifecycle(t *testing.T) {
	...
	userID := entity.NewUserID()
	s, _ := entity.NewSession(userID.UUID(), entity.DeviceDesktop, "10.0.0.1", "TestAgent", "gct-client")
