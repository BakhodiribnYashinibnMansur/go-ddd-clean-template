...
func mustUser(t *testing.T, phone entity.Phone, pw entity.Password, opts ...entity.UserOption) *entity.User {
	t.Helper()
	u, err := entity.NewUser(phone, pw, opts...)
	require.NoError(t, err)
	return u
}
...
func TestNewUser_WithOptions(t *testing.T) {
	...
	u := mustUser(t, phone, pw,
		entity.WithEmail(email),
		entity.WithUsername("john"),
		entity.WithRoleID(roleID),
		entity.WithAttributes(map[string]string{"level": "5"}),
	)
...
func TestUser_AddSession(t *testing.T) {
	...
	s, err := u.AddSession(entity.DeviceDesktop, "1.2.3.4", "TestAgent", "gct-client")
	require.NoError(t, err)
	if s == nil {
...
func TestUser_AddSession_MaxReached(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		_, err := u.AddSession(entity.DeviceMobile, "1.1.1.1", "Agent", "gct-client")
		if err != nil {
...
func TestUser_RemoveSession(t *testing.T) {
	...
	s, _ := u.AddSession(entity.DeviceDesktop, "1.1.1.1", "Agent", "gct-client")
...
func TestUser_RevokeAllSessions(t *testing.T) {
	...
	pw := mustPassword(t, "SecureP@ss1")
	u := mustUser(t, phone, pw)
	u.AddSession(entity.DeviceDesktop, "1.1.1.1", "A1", "gct-client")
	u.AddSession(entity.DeviceMobile, "2.2.2.2", "A2", "gct-client")
