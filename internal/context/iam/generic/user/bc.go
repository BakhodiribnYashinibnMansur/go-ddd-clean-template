...
func NewBoundedContext(pool *pgxpool.Pool, eventBus application.EventBus, committer *outbox.EventCommitter, l logger.Log, jwtCfg command.JWTConfig, maxSessionsFn ...func(ctx context.Context) int) *BoundedContext {
	...
	return &BoundedContext{
		...
		SignUp:          command.NewSignUpHandler(writeRepo, readRepo, pool, eventBus, l),
		SignOut:         command.NewSignOutHandler(writeRepo, pool, eventBus, l),
		ApproveUser:     command.NewApproveUserHandler(writeRepo, pool, eventBus, l),
		ChangeRole:      command.NewChangeRoleHandler(writeRepo, pool, eventBus, l),
		BulkAction:      command.NewBulkActionHandler(writeRepo, pool, eventBus, l),
		RevokeAll:       command.NewRevokeAllSessionsHandler(writeRepo, pool, eventBus, l),
