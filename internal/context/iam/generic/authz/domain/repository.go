...
type PermissionRepository interface {
	...
	FindByID(ctx context.Context, id PermissionID) (*Permission, error)
...
type PolicyRepository interface {
	...
	FindByPermissionID(ctx context.Context, permissionID PermissionID) ([]*Policy, error)
}
...
type RolePermissionRepository interface {
	Assign(ctx context.Context, roleID RoleID, permissionID PermissionID) error
	Revoke(ctx context.Context, roleID RoleID, permissionID PermissionID) error
}
...
type PermissionScopeRepository interface {
	Assign(ctx context.Context, permissionID PermissionID, path, method string) error
	Revoke(ctx context.Context, permissionID PermissionID, path, method string) error
}
...
type PermissionView struct {
	...
	ParentID    *PermissionID `json:"parent_id,omitempty"`
	Name        string        `json:"name"`
	Description *string       `json:"description,omitempty"`
}
...
type AuthzReadRepository interface {
	GetRole(ctx context.Context, id RoleID) (*RoleView, error)
	ListRoles(ctx context.Context, pagination shared.Pagination) ([]*RoleView, int64, error)
	GetPermission(ctx context.Context, id PermissionID) (*PermissionView, error)
	...
	CheckAccess(ctx context.Context, roleID RoleID, path, method string, evalCtx EvaluationContext) (bool, error)
	FindPoliciesByPermissionIDs(ctx context.Context, permissionIDs []PermissionID) ([]*Policy, error)
}
