...
func TestNewFeatureFlag(t *testing.T) {
	...
	ff, _ := entity.NewFeatureFlag("dark-mode", "dark_mode", "Enable dark mode", "bool", "false", 50)
...
func TestFeatureFlag_Evaluate_InactiveReturnsDefault(t *testing.T) {
	...
	ff, _ := entity.NewFeatureFlag("test", "test_flag", "desc", "bool", "false", 100)
...
func TestFeatureFlag_Evaluate_RuleGroupMatch(t *testing.T) {
	...
	ff, _ := entity.NewFeatureFlag("test", "test_flag", "desc", "bool", "false", 0)
	ff.Activate()

	rg := entity.NewRuleGroup(ff.ID(), "beta", "true", 1)
	rg.AddCondition(entity.NewCondition("country", "eq", "US"))
	ff.AddRuleGroup(rg)
...
func TestFeatureFlag_Evaluate_PriorityOrder(t *testing.T) {
	...
	ff, _ := entity.NewFeatureFlag("test", "test_flag", "desc", "string", "default", 0)
	ff.Activate()

	rg1 := entity.NewRuleGroup(ff.ID(), "low-priority", "variation-B", 2)
	rg1.AddCondition(entity.NewCondition("country", "eq", "US"))
	ff.AddRuleGroup(rg1)

	rg2 := entity.NewRuleGroup(ff.ID(), "high-priority", "variation-A", 1)
	rg2.AddCondition(entity.NewCondition("country", "eq", "US"))
	ff.AddRuleGroup(rg2)
...
func TestFeatureFlag_Evaluate_MultipleRuleGroups_AND(t *testing.T) {
	...
	rg := entity.NewRuleGroup(ff.ID(), "multi-condition", "true", 1)
	rg.AddCondition(entity.NewCondition("country", "eq", "US"))
	rg.AddCondition(entity.NewCondition("age", "gte", "18"))
	ff.AddRuleGroup(rg)
...
func TestFeatureFlag_ReconstructWithRuleGroups(t *testing.T) {
	...
	rg := entity.ReconstructRuleGroup(uuid.New(), id.UUID(), "beta", "true", 1, now, now, []entity.Condition{
		entity.ReconstructCondition(uuid.New(), uuid.New(), "country", "eq", "US"),
	})

	ff := entity.ReconstructFeatureFlag(id.UUID(), now, now, nil, "test", "test_flag", "desc", "bool", "false", 50, true, []*entity.RuleGroup{rg})
