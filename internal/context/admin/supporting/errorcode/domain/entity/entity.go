...
import (
	...
	"gct/internal/context/admin/supporting/errorcode/domain/event"
...
func NewErrorCode(
	...
	ec.AddEvent(event.NewErrorCodeCreated(ec.ID(), code, message, httpStatus))
	return ec
}
...
func (ec *ErrorCode) Update(
	...
	ec.Touch()
	ec.AddEvent(event.NewErrorCodeUpdated(ec.ID(), ec.code, message, httpStatus))
}
