...
type ErrorCodeRepository interface {
	...
	FindByID(ctx context.Context, id ErrorCodeID) (*ErrorCode, error)
	Delete(ctx context.Context, id ErrorCodeID) error
}

// ErrorCodeView is a read-model DTO for error codes.
type ErrorCodeView struct {
	ID         ErrorCodeID `json:"id"`
...
type ErrorCodeReadRepository interface {
	FindByID(ctx context.Context, id ErrorCodeID) (*ErrorCodeView, error)
