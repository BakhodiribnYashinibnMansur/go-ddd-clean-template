...
// MarshalJSON serializes the DataExportID as a canonical UUID string.
func (id DataExportID) MarshalJSON() ([]byte, error) {
	return json.Marshal(id.String())
}

// UnmarshalJSON parses a DataExportID from a JSON UUID string.
func (id *DataExportID) UnmarshalJSON(data []byte) error {
	var s string
	if err := json.Unmarshal(data, &s); err != nil {
		return err
	}
	parsed, err := ParseDataExportID(s)
	if err != nil {
		return err
	}
	*id = parsed
...
func (id DataExportID) Value() (driver.Value, error) {
	return uuid.UUID(id).Value()
}

// Scan implements sql.Scanner for SQL driver interop.
func (id *DataExportID) Scan(src any) error {
	var u uuid.UUID
