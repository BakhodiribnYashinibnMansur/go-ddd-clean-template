...
func TestDataExportID_RoundTrip(t *testing.T) {
	...
	parsed, err := entity.ParseDataExportID(id.String())
	if err != nil {
...
func TestParseDataExportID_Invalid(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if _, err := entity.ParseDataExportID(tc.in); err == nil {
...
func TestDataExportID_IsZero(t *testing.T) {
	...
	var zero entity.DataExportID
	if !zero.IsZero() {
		t.Fatal("zero-valued DataExportID should report IsZero()")
	}

	nonZero := entity.DataExportID(uuid.New())
	if nonZero.IsZero() {
...
func TestDataExportID_Distinct(t *testing.T) {
	...
	a := entity.NewDataExportID()
	b := entity.NewDataExportID()
	if a == b {
