...
func TestNewDataExport(t *testing.T) {
	...
	userID := uuid.New()
	de := entity.NewDataExport(userID, "users", "csv")
	...
	if de.Status() != entity.ExportStatusPending {
...
func TestDataExport_Complete(t *testing.T) {
	...
	de := entity.NewDataExport(uuid.New(), "orders", "xlsx")
	de.StartProcessing()

	if de.Status() != entity.ExportStatusProcessing {
		t.Fatalf("expected status PROCESSING, got %s", de.Status())
	}

	de.Complete("https://example.com/export.xlsx")

	if de.Status() != entity.ExportStatusCompleted {
...
func TestDataExport_Fail(t *testing.T) {
	...
	de := entity.NewDataExport(uuid.New(), "logs", "json")
	de.Fail("disk full")

	if de.Status() != entity.ExportStatusFailed {
...
func TestReconstructDataExport(t *testing.T) {
	...
	id := entity.NewDataExportID()
	userID := uuid.New()
	now := time.Now()
	fileURL := "https://example.com/file.csv"

	de := entity.ReconstructDataExport(id.UUID(), now, now, userID, "users", "csv", entity.ExportStatusCompleted, &fileURL, nil)
