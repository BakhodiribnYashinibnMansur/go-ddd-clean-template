...
func TestIntegrationID_RoundTrip(t *testing.T) {
	...
	id := entity.NewIntegrationID()
	if id.IsZero() {
		t.Fatal("newly generated IntegrationID should not be zero")
	}

	parsed, err := entity.ParseIntegrationID(id.String())
	if err != nil {
...
func TestParseIntegrationID_Invalid(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if _, err := entity.ParseIntegrationID(tc.in); err == nil {
...
func TestIntegrationID_IsZero(t *testing.T) {
	...
	var zero entity.IntegrationID
	if !zero.IsZero() {
		t.Fatal("zero-valued IntegrationID should report IsZero()")
	}

	nonZero := entity.IntegrationID(uuid.New())
	if nonZero.IsZero() {
...
func TestIntegrationID_Distinct(t *testing.T) {
	...
	a := entity.NewIntegrationID()
	b := entity.NewIntegrationID()
	if a == b {
