...
func TestNewAPIKey(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			got, err := entity.NewAPIKey(tt.input)
			if tt.wantErr {
				if err == nil {
					t.Fatalf("expected error for %q, got nil", tt.input)
				}
				if !errors.Is(err, entity.ErrInvalidAPIKey) {
...
func TestAPIKey_Equal(t *testing.T) {
	t.Parallel()
	a, _ := entity.NewAPIKey("abcdefghijklmnop")
	b, _ := entity.NewAPIKey("abcdefghijklmnop")
	c, _ := entity.NewAPIKey("zzzzzzzzzzzzzzzz")
	if !a.Equal(b) {
