...
func TestNewIntegration(t *testing.T) {
	...
	i, _ := entity.NewIntegration("Stripe", "payment", "sk_test_123", "https://hooks.example.com", true, config)
...
func TestIntegration_SetJWT_RejectsInvalidTTLs(t *testing.T) {
	t.Parallel()
	i, _ := entity.NewIntegration("Stripe", "payment", "sk", "https://e", true, nil)

	err := i.SetJWT([]byte("h"), 0, 1*time.Hour, "pem", "kid", entity.BindingModeWarn, 0)
	require.Error(t, err)

	err = i.SetJWT([]byte("h"), 1*time.Hour, -1, "pem", "kid", entity.BindingModeWarn, 0)
	require.Error(t, err)
}
...
func TestIntegration_SetJWT_AllValidModes(t *testing.T) {
	t.Parallel()
	for _, mode := range []string{entity.BindingModeOff, entity.BindingModeWarn, entity.BindingModeStrict} {
		i, _ := entity.NewIntegration("Stripe", "payment", "sk", "https://e", true, nil)
...
func TestIntegration_RotateJWTKey(t *testing.T) {
	t.Parallel()
	i, _ := entity.NewIntegration("Stripe", "payment", "sk", "https://e", true, nil)
	_ = i.SetJWT([]byte("h"), 1*time.Hour, 24*time.Hour, "first-pem", "kid-1", entity.BindingModeWarn, 0)
