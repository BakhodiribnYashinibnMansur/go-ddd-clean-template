...
func TestNewJWTAPIKey_Valid(t *testing.T) {
	t.Parallel()
	k, err := entity.NewJWTAPIKey("0123456789abcdef0123456789abcdef") // 32 chars
...
func TestNewJWTAPIKey_RejectsShortInput(t *testing.T) {
	t.Parallel()
	k, err := entity.NewJWTAPIKey("too-short")
	require.Error(t, err)
	require.True(t, errors.Is(err, entity.ErrInvalidJWTAPIKey))
...
func TestJWTAPIKey_HashIsDeterministic(t *testing.T) {
	...
	k, err := entity.NewJWTAPIKey(plain)
	require.NoError(t, err)
...
func TestVerifyJWTAPIKeyHash_Match(t *testing.T) {
	...
	assert.True(t, entity.VerifyJWTAPIKeyHash(plain, pepper, stored))
}

func TestVerifyJWTAPIKeyHash_Mismatch(t *testing.T) {
	t.Parallel()
	pepper := []byte("verification-pepper-0000000000000")
	k, _ := entity.NewJWTAPIKey("0123456789abcdef0123456789abcdef")
	stored := k.Hash(pepper)

	assert.False(t, entity.VerifyJWTAPIKeyHash("fedcba9876543210fedcba9876543210", pepper, stored))
}

func TestGenerateJWTAPIKey(t *testing.T) {
	t.Parallel()
	k, err := entity.GenerateJWTAPIKey()
	require.NoError(t, err)
	assert.GreaterOrEqual(t, len(k.Reveal()), 32)

	// Two generations should differ.
	k2, err := entity.GenerateJWTAPIKey()
	require.NoError(t, err)
