...
func TestNewFunctionMetric(t *testing.T) {
	...
	fm := entity.NewFunctionMetric("UserService.Create", 150.5, false, nil)
...
func TestNewFunctionMetric_WithPanic(t *testing.T) {
	...
	fm := entity.NewFunctionMetric("Handler.Process", 0.5, true, &panicErr)
...
func TestReconstructFunctionMetric(t *testing.T) {
	...
	fm := entity.ReconstructFunctionMetric(id.UUID(), createdAt, "TestFunc", 42.0, true, &panicErr)
