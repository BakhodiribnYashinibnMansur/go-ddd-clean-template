...
import (
	...
	"gct/internal/context/ops/generic/systemerror/domain/event"
...
func NewSystemError(code, message, severity string) *SystemError {
	...
	se.AddEvent(event.NewSystemErrorRecorded(se.ID(), code, severity))
	return se
}
...
func (se *SystemError) Resolve(resolvedBy uuid.UUID) {
	...
	se.Touch()
	se.AddEvent(event.NewSystemErrorResolved(se.ID(), resolvedBy))
}
