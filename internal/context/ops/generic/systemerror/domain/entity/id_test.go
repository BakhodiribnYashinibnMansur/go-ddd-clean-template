...
func TestSystemErrorID_RoundTrip(t *testing.T) {
	...
	id := entity.NewSystemErrorID()
	if id.IsZero() {
		t.Fatal("newly generated SystemErrorID should not be zero")
	}

	parsed, err := entity.ParseSystemErrorID(id.String())
	if err != nil {
...
func TestParseSystemErrorID_Invalid(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if _, err := entity.ParseSystemErrorID(tc.in); err == nil {
...
func TestSystemErrorID_IsZero(t *testing.T) {
	...
	var zero entity.SystemErrorID
	if !zero.IsZero() {
		t.Fatal("zero-valued SystemErrorID should report IsZero()")
	}

	nonZero := entity.SystemErrorID(uuid.New())
	if nonZero.IsZero() {
...
func TestSystemErrorID_Distinct(t *testing.T) {
	...
	a := entity.NewSystemErrorID()
	b := entity.NewSystemErrorID()
	if a == b {
