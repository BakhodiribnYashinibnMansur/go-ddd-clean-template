...
func TestNewBoundedContext(t *testing.T) {
	...
	if bc.CreateSystemError == nil {
		t.Error("CreateSystemError handler not wired")
	}
	if bc.ResolveError == nil {
		t.Error("ResolveError handler not wired")
	}
	if bc.GetSystemError == nil {
		t.Error("GetSystemError handler not wired")
	}
	if bc.ListSystemErrors == nil {
		t.Error("ListSystemErrors handler not wired")
	}
}
