...
func TestRateLimitID_RoundTrip(t *testing.T) {
	...
	id := entity.NewRateLimitID()
	if id.IsZero() {
		t.Fatal("newly generated RateLimitID should not be zero")
	}

	parsed, err := entity.ParseRateLimitID(id.String())
	if err != nil {
...
func TestParseRateLimitID_Invalid(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if _, err := entity.ParseRateLimitID(tc.in); err == nil {
...
func TestRateLimitID_IsZero(t *testing.T) {
	...
	var zero entity.RateLimitID
	if !zero.IsZero() {
		t.Fatal("zero-valued RateLimitID should report IsZero()")
	}

	nonZero := entity.RateLimitID(uuid.New())
	if nonZero.IsZero() {
...
func TestRateLimitID_Distinct(t *testing.T) {
	...
	a := entity.NewRateLimitID()
	b := entity.NewRateLimitID()
	if a == b {
