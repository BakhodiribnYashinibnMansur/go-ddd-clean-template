...
func TestIPRuleID_RoundTrip(t *testing.T) {
	...
	id := entity.NewIPRuleID()
	if id.IsZero() {
		t.Fatal("newly generated IPRuleID should not be zero")
	}

	parsed, err := entity.ParseIPRuleID(id.String())
	if err != nil {
...
func TestParseIPRuleID_Invalid(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if _, err := entity.ParseIPRuleID(tc.in); err == nil {
...
func TestIPRuleID_IsZero(t *testing.T) {
	...
	var zero entity.IPRuleID
	if !zero.IsZero() {
		t.Fatal("zero-valued IPRuleID should report IsZero()")
	}

	nonZero := entity.IPRuleID(uuid.New())
	if nonZero.IsZero() {
...
func TestIPRuleID_Distinct(t *testing.T) {
	...
	a := entity.NewIPRuleID()
	b := entity.NewIPRuleID()
	if a == b {
