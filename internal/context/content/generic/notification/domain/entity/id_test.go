...
func TestNotificationID_RoundTrip(t *testing.T) {
	...
	id := entity.NewNotificationID()
	if id.IsZero() {
		t.Fatal("newly generated NotificationID should not be zero")
	}

	parsed, err := entity.ParseNotificationID(id.String())
	if err != nil {
...
func TestParseNotificationID_Invalid(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if _, err := entity.ParseNotificationID(tc.in); err == nil {
...
func TestNotificationID_IsZero(t *testing.T) {
	...
	var zero entity.NotificationID
	if !zero.IsZero() {
		t.Fatal("zero-valued NotificationID should report IsZero()")
	}

	nonZero := entity.NotificationID(uuid.New())
	if nonZero.IsZero() {
...
func TestNotificationID_Distinct(t *testing.T) {
	...
	a := entity.NewNotificationID()
	b := entity.NewNotificationID()
	if a == b {
