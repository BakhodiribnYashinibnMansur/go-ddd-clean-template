...
func TestTranslationID_RoundTrip(t *testing.T) {
	...
	id := entity.NewTranslationID()
	if id.IsZero() {
		t.Fatal("newly generated TranslationID should not be zero")
	}

	parsed, err := entity.ParseTranslationID(id.String())
	if err != nil {
...
func TestParseTranslationID_Invalid(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if _, err := entity.ParseTranslationID(tc.in); err == nil {
...
func TestTranslationID_IsZero(t *testing.T) {
	...
	var zero entity.TranslationID
	if !zero.IsZero() {
		t.Fatal("zero-valued TranslationID should report IsZero()")
	}

	nonZero := entity.TranslationID(uuid.New())
	if nonZero.IsZero() {
...
func TestTranslationID_Distinct(t *testing.T) {
	...
	a := entity.NewTranslationID()
	b := entity.NewTranslationID()
	if a == b {
