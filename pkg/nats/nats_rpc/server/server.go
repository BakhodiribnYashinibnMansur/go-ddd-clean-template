...
func (s *Server) Shutdown() error {
	...
	if err != nil && !errors.Is(err, context.Canceled) {
		s.logger.Error("nats_rpc server - Server - Shutdown - s.eg.Wait", zap.Error(err))
	...
	if s.subscription != nil {
		...
		if err != nil {
			s.logger.Error("nats_rpc server - Server - Shutdown - s.conn.Subscription.Unsubscribe", zap.Error(err))
...
func (s *Server) handleMessage(msg *nats.Msg) {
	...
	if err != nil {
		...
		s.logger.Error("nats_rpc server - Server - handleMessage - callHandler", zap.Error(err))
	...
	if err != nil {
		s.logger.Error("nats_rpc server - Server - handleMessage - json.Marshal", zap.Error(err))
...
func (s *Server) publish(msg *nats.Msg, body []byte, status string) {
	...
	if err != nil {
		s.logger.Error("nats_rpc server - Server - publish - msg.Respond", zap.Error(err))
	}
}
