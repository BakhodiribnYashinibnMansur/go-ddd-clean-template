...
func (p *Postgres) Listen(ctx context.Context, channel string, onNotify func(payload string) error, l logger.Log) {
	if p.Pool == nil {
	...
	if err != nil {
		l.Errorw("failed to acquire connection for listener", zap.Error(err))
		return
	}
	defer conn.Release()
	...
	if err != nil {
		l.Errorw("failed to listen on channel", "channel", channel, zap.Error(err))
		return
	}

	l.Infow("started listening for notifications", "channel", channel)

	for {
		select {
		case <-ctx.Done():
			l.Infow("stopping listener", "channel", channel)
			return
		default:
			...
			if err != nil {
				...
				l.Errorw("error waiting for notification", zap.Error(err))
				// Connection might be broken, exit the loop.
				// In a real-world scenario, we might want a retry mechanism with backoff.
				return
			}

			l.Infow("received notification", "channel", notification.Channel, "payload", notification.Payload)

			if notification.Payload != "" {
				if err := onNotify(notification.Payload); err != nil {
					l.Errorw("failed to handle notification", "payload", notification.Payload, zap.Error(err))
				}
			}
		}
	}
