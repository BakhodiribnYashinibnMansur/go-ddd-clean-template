...
func TestSanitizePhone_Property(t *testing.T) {
	...
	propertySubsequence := func(input string) bool {
		...
		// Check if outDigits is a subsequence of input
		inputIdx := 0
		for _, outChar := range outDigits {
			found := false
			for i := inputIdx; i < len(input); i++ {
				// We must match loose bytes or runes?
				// SanitizePhone loop iterates 'range phone' which is runes.
				// But we are searching. Let's convert to runes for safety.
				shouldMatch := outChar
				inputRune := rune(input[i])
				if inputRune == shouldMatch {
					inputIdx = i + 1
					found = true
					break
				}
