...
func (s *Server) Shutdown() error {
	...
	if err != nil && !errors.Is(err, context.Canceled) {
		s.logger.Error("rmq_rpc server - Server - Shutdown - s.eg.Wait", zap.Error(err))
	...
	if err != nil {
		s.logger.Error("rmq_rpc server - Server - Shutdown - s.Connection.Close", zap.Error(err))
...
func (s *Server) ack(d *amqp.Delivery, multiple bool) {
	err := d.Ack(multiple)
	if err != nil {
		s.logger.Error("rmq_rpc server - Server - ack - d.Ack", zap.Error(err))
	}
}
